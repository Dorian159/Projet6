...
class Module {
	...
	public function register_controls( $element ) {
		...
		$element->add_control(
			'premium_shapes_data',
			array(
				'label'   => esc_html__( 'Shapes Data', 'premium-addons-for-elementor' ),
				'type'    => Controls_Manager::HIDDEN,
				'default' => $this->svg_shapes,
			)
		);
	...
	public function before_render( $element ) {
		...
		if ( 'yes' === $divider_enabled ) {
			...
			if ( 'default' !== $source ) {
				...
				$shape = '' !== $settings['premium_gdivider_defaults'] ? $this->get_svg_html( $settings['premium_gdivider_defaults'] ) : '';
			}
	 ...
	 * @since 4.10.11
	 */
	public function set_svg_shapes_info() {

		$shape_source = 'https://premiumtemplates.io/wp-content/uploads/premium-shape-divider/shapes.json?nocache=5';

		$shapes = rplg_urlopen( $shape_source );

		if ( isset( $shapes['code'] ) ) {
			if ( 404 === $shapes['code'] ) {
				return '';
			}
		} else {
		...
		$shapes = json_decode( $shapes['data'], true );

		$this->svg_shapes = $shapes;
	}
	 ...
	 * @since 4.10.11
	 *
	 * @param string $index  svg index.
	 *
	 * @return string
	 */
	public function get_svg_html( $index ) {

		$svg_shapes = $this->svg_shapes;

		return $svg_shapes[ $index ]['imagesmall'];
	}
